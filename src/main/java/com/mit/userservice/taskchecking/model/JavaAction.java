...
public class JavaAction implements Action {
    ...
    public void compile() {
        ...
        String[] compilation = new String[]{
                "bash",
                "-c",
                "bash java-compilation.sh " + pathToFolderWithUSerSolution + " " + filename +
    ...
    public void prepareSystemToStartChecker() {
        ...
        Writer output = null;
        fileName = parseClassName(solution);
        try {
            ...
            file = new File(pathToFolderWithUSerSolution + fileName + ".java");
    ...
    private String parseClassName(Solution solution) {
        String pattern = "(?<=\\n|\\A)(?:public\\s)?(class)\\s([^\\n\\s]*)";
        Pattern p = Pattern.compile(pattern);
        Matcher m = p.matcher(solution.getSolutionText());
        if (m.find()) {
            if (m.group(2) != null) {
                return m.group(2);
            }
